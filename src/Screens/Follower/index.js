...
import GetFollowerMiddleware from '../../Store/Middleware/GetFollowerMiddleware';
...
class Follower extends Component {
  ...
  onPressLoadMore = () => {
    this.setState({ loader: true }, () => {
      const { getFollowerData } = this.props;
      this.props
        .getFollowerUser(getFollowerData.next_page_url, '')
        .then(() => {
  ...
  renderLoaderMoreButton = () => {
    ...
    return getFollowerData.next_page_url ? (
      loader ? (
        <ActivityIndicator
          size={'large'}
          color={Colors.BLUE}
  ...
  FollowUser = (item) => {
    let followerCopy = this.props.getFollowerData_list

    let index = followerCopy.findIndex(val => val.id === item.id)

    if (!followerCopy[index].isFollow) {
      let updateItem = { ...item, isFollow: true, }


      followerCopy.splice(index, 1, updateItem)
    ...
    this.setState({ followerCopy })
    this.props
      .followUser({ id: item.id })
  };

  render() {
    const { getFollowerData, getFollowerData_list, loader } = this.props;
    // console.warn('getFollowerData_list', getFollowerData_list.isTrue)

    // console.warn('==========================>>>>>', getFollowerData_list)
    return (
      <View style={styles.container}>
        ...
        {getFollowerData_list && getFollowerData_list?.length ? (
          <FlatList
            refreshControl={
              <RefreshControl
                refreshing={loader}
                onRefresh={this.onRefreshGetFollower}
              />
            }
            style={styles.flex1}
            showsVerticalScrollIndicator={false}
            // data={[1, 1, 1, 1, 1, 1, 1, 1, 1]}
            data={getFollowerData_list}
...
const mapStateToProps = state => {
  return {
    ...
    getFollowerData: state.GetFollowerReducer.getFollowerData,
    getFollowerData_list: state.GetFollowerReducer.getFollowerData_list,
    followUserData: state.FollowReducer.followUserData,
  };
};
const mapsDispatchToProps = dispatch => {
  return {
    getFollowerUser: payload => dispatch(GetFollowerMiddleware.getFollowerUser(payload)),
    followUser: payload => dispatch(FollowMiddleware.followUser(payload)),

  };
};
export default connect(mapStateToProps, mapsDispatchToProps)(Follower);
